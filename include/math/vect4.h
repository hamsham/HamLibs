...
class vec4 {
	...
	public:
		...
		//constructors
		vec4			();
		vec4			(numType inVect[4]);
		vec4			(const vec4<numType>& invect);
		vec4			(numType inX, numType inY, numType inZ, numType inW);
		~vec4			(){}
		...
		vec4			operator 		--		(int);
		bool			operator		==		(const vec4<numType>& compare); //comparisons
		bool			operator		!=		(const vec4<numType>& compare);
		bool			operator		<		(const vec4<numType>& compare);
		bool			operator		>		(const vec4<numType>& compare);
		bool			operator		<=		(const vec4<numType>& compare);
		bool			operator		>=		(const vec4<numType>& compare);
...
template <class numType> HL_INLINE
vec4<numType>::vec4(numType inVect[4]) :
	x(inVect[X]), y(inVect[Y]),  z(inVect[Z]), w(inVect[W])
{}

template <class numType> HL_INLINE
vec4<numType>::vec4(const vec4<numType>& inVect) :
	x(inVect.x), y(inVect.y), z(inVect.z), w(inVect.w)
{}
...
//comparisons
template <class numType> HL_INLINE
bool vec4<numType>::operator== (const vec4<numType>& compare) {
	return (	x == compare.x &&
				y == compare.y &&
				z == compare.z &&
				w == compare.w
